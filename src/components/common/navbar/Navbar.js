...
function Navbar({ setCurrentChain }) {
  ...
  const checkUserRegistered = async () => {
    try {
      ...
      console.log(endpoint, "endpointg url");

      const headers = { "Content-Type": "application/json" };
      const response = await makeCall(endpoint, {}, headers, "get");
      console.log(response, "checking the response things");

      if (!response.data) {
        console.log("we are in in here");
      ...
      console.log(error, "checking ooooooo");
  ...
  useEffect(() => {
    if (switchWalletType === "local" && address) {
      console.log("callled oooosisisisis");
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link to="/" className="flex items-center gap-2">
            ...
            <h1 className="hidden text-lg font-extrabold text-white md:block">
              Royal Bets
            </h1>
          </Link>
        </div>
          ...
          {address && (
            <div className="flex flex-col items-center max-w-[50px] ml-2">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                ...
                />
                <div
                  className="w-12 h-7 bg-gray-700 peer-focus:outline-none rounded-lg peer 
